...
import { supabaseNodes, supabaseEdges } from '@site/src/components/ERDEmbed/schema-data-supabase';
import { purePgNodes, purePgEdges } from '@site/src/components/ERDEmbed/schema-data-purepg';

# Database Schema

Halaman ini menyajikan struktur database ERD Builder Pro secara visual. Diagram di bawah ini merepresentasikan tabel-tabel utama yang tersimpan di database PostgreSQL serta bagaimana data tersebut saling terhubung.

Aplikasi mendukung dua mode database:

| Mode | Keterangan |
|------|-----------|
| **Supabase** | Menggunakan Supabase Auth (`auth.users`) dengan tipe ID `BigInt`, schema `auth` dan `public` |
| **Pure PostgreSQL** | Menggunakan model User lokal, tipe ID `Int`, tanpa dependensi Supabase Auth — cocok untuk development lokal/self-hosted |

## Visualisasi Struktur Database

Gunakan diagram interaktif di bawah ini untuk memahami alur data antar tabel. Pilih tab yang sesuai dengan mode database yang Anda gunakan.

<Tabs>
  <TabItem value="supabase" label="Supabase" default>

<ERDEmbed height="550px" nodes={supabaseNodes} edges={supabaseEdges} />

  </TabItem>
  <TabItem value="purepg" label="Pure PostgreSQL">

<ERDEmbed height="550px" nodes={purePgNodes} edges={purePgEdges} />
...
Pada mode **Supabase**, tabel auth dikelola oleh Supabase Auth (`auth.users`) dan tidak tampil di diagram. Pada mode **Pure PostgreSQL**, terdapat tabel `users` dan `sessions` lokal sebagai pengganti auth Supabase.

---

## Kode SQL (DDL) Lengkap

Bagi para pengembang yang ingin melakukan instalasi mandiri atau migrasi database, silakan gunakan skema SQL lengkap di bawah ini sesuai mode yang dipilih.

<details>
<summary>Skema SQL Supabase (DDL + RLS Policies + Triggers + Seed)</summary>
...
<details>
<summary>Skema SQL Pure PostgreSQL (DDL + Foreign Keys + Indexes)</summary>
...
| Tabel | Schema | Kategori | Deskripsi |
|-------|--------|----------|-----------|
| `users` | Supabase: `auth` / Pure: `public` | Auth | Data pengguna (Supabase: `auth.users` via Supabase Auth; Pure: model lokal dengan email/password) |
| `sessions` | `public` (Pure only) | Auth | Session token untuk auth lokal (hanya di mode Pure PostgreSQL) |
| `projects` | `public` | Project | Kontainer utama untuk semua file (diagrams, notes, drawings, flowcharts) |
| `diagrams` | `public` | Diagram | Tabel induk untuk file ERD, menyimpan viewport, metadata sharing, dan pengaturan source |
| `entities` | `public` | ERD | Menyimpan tabel-tabel individual di dalam diagram ERD |
| `columns` | `public` | ERD | Menyimpan field/kolom dari setiap entity |
| `relationships` | `public` | ERD | Menyimpan garis relasi antar entity (one-to-one, one-to-many, many-to-many) |
| `entity_changes` | `public` | Audit | Riwayat perubahan entity untuk fitur version history / snapshot |
| `notes` | `public` | Konten | Catatan teks kaya (rich text) dengan soft-delete dan sharing |
| `drawings` | `public` | Konten | Gambar bebas (freeform drawing) berbasis JSON |
| `flowcharts` | `public` | Konten | Flowchart dengan data nodes/edges dalam format JSON |
| `backups` | `public` | Utilitas | Riwayat backup database per pengguna |
| `ai_providers` | `public` | AI | Daftar penyedia AI (OpenAI, Anthropic, dll.) |
| `ai_models` | `public` | AI | Model AI yang tersedia per provider |
| `user_ai_configs` | `public` | AI | Konfigurasi AI per pengguna (provider, model, API key) |
| `ai_chat_sessions` | `public` | AI | Sesi percakapan AI Chat |
| `ai_chat_messages` | `public` | AI | Pesan individual dalam sesi AI Chat |
| `ai_system_prompts` | `public` | AI | Prompt sistem untuk AI (built-in & kustom) |
| `user_ai_rules` | `public` | AI | Aturan AI per view (misal: aturan untuk ERD, flowchart, dll.) |
| `user_preferences` | `public` | Utilitas | Preferensi aplikasi desktop per pengguna |
| `db_accounts` | `public` | Integrasi | Kredensial koneksi database eksternal (desktop-only) |
| `db_catalogs` | `public` | Integrasi | Database spesifik dalam sebuah db_account |

### Perbedaan Utama Antara Supabase dan Pure PostgreSQL

| Aspek | Supabase | Pure PostgreSQL |
|-------|----------|----------------|
| **User model** | `auth.users` (dari Supabase) | `users` lokal (UUID, email, password) |
| **Auth tables** | ~30 tabel auth (identities, sessions, mfa, sso, dll.) | 1 tabel `sessions` |
| **ID type** | `BIGSERIAL` / `BigInt` | `SERIAL` / `Int` |
| **Timestamps** | `TIMESTAMPTZ(6)` dengan presisi | `TIMESTAMP(3)` / `TIMESTAMPTZ` |
| **Project** | tanpa `color`, `updatedAt`, `version` | memiliki `color`, `updated_at`, `_version` |
| **EntityChange.changes** | `JSONB` | `TEXT` (serialized JSON) |
| **Backup.file_size** | `BIGINT` | `INTEGER` |
| **Cocok untuk** | Production dengan Supabase | Development lokal / self-hosted |
